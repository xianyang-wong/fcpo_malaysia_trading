    ...
    def getRreturn(self,df):
        ...
        for IndividualCount in range(0,20):  
            if self.DfFitness.holding[IndividualCount] !=0:
                tmpProfit += self.DfFitness.iloc[IndividualCount]['holding'] * (df.Low[self.EndIndex]  - self.DfFitness.iloc[IndividualCount]['lastTradeValue'])
        return ((tmpProfit + self.DfFitness.riskfree - self.DfFitness.cost  )/self.DfFitness.capital)
